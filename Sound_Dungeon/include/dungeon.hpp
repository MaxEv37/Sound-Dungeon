...
public:
    ...
    // **Public functions for retrieving rooms**
    const std::vector<std::unique_ptr<Room>>& getRooms();
    Room* getRoomAtID(unsigned int id) const;
...
private:
    ...
    std::map<RoomSubType, std::vector<std::string>> roomSounds = {
        {RoomSubType::b_Treasure, {"treasure_1.wav"}},
        {RoomSubType::b_Rest, {"campfire_crackle_1.wav"}},
        {RoomSubType::b_Garden, {"garden_1.wav"}},
        {RoomSubType::h_ScytheTrap, {"campfire_crackle_1.wav"}},
        ...
        {RoomSubType::v_endRoom, {"finger_click_4.wav"}}
    };
